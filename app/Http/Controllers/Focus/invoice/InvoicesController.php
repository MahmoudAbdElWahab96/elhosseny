...
class InvoicesController extends Controller
{
    ...
    public function store(CreateInvoiceRequest $request)
    {
        ...
        $invoice = $request->only(['customer_id', 'tid', 'refer',
            'invoicedate', 'invoiceduedate', 'recur_after', 'sub',
            'notes', 'subtotal', 'shipping', 'tax', 'discount',
            'discount_rate', 'after_disc', 'currency', 'total',
            'tax_format', 'discount_format', 'ship_tax', 'ship_tax_type',
            'ship_rate', 'term_id', 'tax_id','sub_tax_id', 'p','ordered_supply_id']);

        $invoice_items = $request->only(['product_id', 'product_name',
    ...
    public function update(EditInvoiceRequest $request, Invoice $invoice_r)
    {
        ...
        //Input received from the request
        $invoice = $request->only(['ordered_supply_id','customer_id', 'id', 'refer', 'invoicedate', 'invoiceduedate', 'notes', 'subtotal', 'shipping', 'tax', 'discount', 'discount_rate', 'after_disc', 'currency', 'total', 'tax_format', 'discount_format', 'ship_tax', 'ship_tax_type', 'ship_rate', 'ship_tax', 'term_id', 'tax_id', 'restock', 'recur_after']);
    ...
    public function pos_store(ManagePosRequest $request, PrinterController $printer)
    {
        ...
        $result = $this->repository->create(compact('invoice', 'invoice_items', 'data2'));
        if ($result) {
            foreach ($invoice_items['product_id'] as $id) {
            ...
            if (isset($result['p'])) $lk .= '<a href="' . route('biller.projects.show', [$result['p']]) . '" class="btn btn-info btn-md"><span class="fa fa-repeat" aria-hidden="true"></span> ' . trans('invoices.return_project') . '  </a> ';
            if ($pay) echo json_encode(array('status' => 'Success', 'message' => trans('alerts.backend.invoices.created')
                . ' <a href="' . route('biller.invoices.show', [$result->id]) .
                '" class="btn btn-info btn-md"><span class="fa fa-eye" aria-hidden="true"></span> ' .
                trans('general.view') . '</a><button onclick="printmyinvoice()" class="btn btn-purple btn-md"><span class="fa fa-print" aria-hidden="true"></span> الفاتورة  </button>  <button onclick="print('.
                ...
                trans('general.preview') . '  </a> <a href="' . route('biller.invoices.pos') .
    ...
    public function delivery(ManagePosRequest $request, RegistersController $register)
    {
        ...
        $accounts = Account::all();
        $input['sub'] = true;
        $last_invoice = Invoice::where('i_class', '=', 2)->latest()->first();
        return view('focus.invoices.delivery.create')->with(array('last_invoice' => $last_invoice, 'sub' => $input['sub'], 'p' => $request->p, 'accounts' => $accounts, 'customer' => $customer))->with(bill_helper(1, 2))->with(product_helper());
    }
    ...
    public function getSubTaxes(Request $request)
    {
        if (app()->getLocale() == 'ar') {
            ...
            $data = Additional::where('tax_id', $request->tax_id)->pluck('name','id');
        }else{

            $data = Additional::where('tax_id', $request->tax_id)->pluck('name_en','id');
        }
