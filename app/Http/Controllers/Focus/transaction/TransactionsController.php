...
class TransactionsController extends Controller
{
    ...
    {
        $screens = [];
        $resource = Account::find($request->account_id);
        $screens = $resource->screen;

        $data = [];
        foreach($screens as $key => $screen){
            
            $cost_centers = $screen->costCenters()->pluck('name_ar', 'id')->toArray();
            if (count($cost_centers) > 0){
                array_push($data, [
                    'screen' => $screen->name_ar,
                    'cost_centers' => $cost_centers
                ]);
            }
        }
        $html = view('focus.transactions._screens',
            [
                'resource' => $resource,
                'data' =>$data
            ])->render();
