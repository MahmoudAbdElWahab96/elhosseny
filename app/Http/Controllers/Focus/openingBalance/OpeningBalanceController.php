...
class OpeningBalanceController extends Controller
{
     ...
     *
     * @param App\Http\Requests\Focus\openingBalance\ManageOpeningBalanceRequest $request
     * @return \App\Http\Responses\ViewResponse
     */
    public function index(ManageOpeningBalanceRequest $request)
    {
        return new ViewResponse('focus.openingBalance.index');
    }

    /**
     * Show the form for creating a new resource.
     *
     * @param CreateOpeningBalanceRequestNamespace $request
     * @return \App\Http\Responses\Focus\openingBalance\CreateResponse
     */
    public function create(StoreOpeningBalanceRequest $request)
    {
        return new CreateResponse('focus.openingBalance.create');
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param StoreOpeningBalanceRequestNamespace $request
    ...
    public function store(StoreOpeningBalanceRequest $request)
    {
        ...
        foreach($input as $opening){
            foreach($opening as $value){

                $openBalance = OpeningBalance::find($value[0]['id']);

                $this->repository->update($openBalance, $value[0]);
            }
        }
       

        return new RedirectResponse(route('biller.opening_balance.index'), ['flash_success' => trans('alerts.backend.openingBalance.created')]);
    }

    // /**
    //  * Show the form for editing the specified resource.
    //  *
    //  * @param App\Models\openingBalance\OpeningBalance $account
     ...
     * Remove the specified resource from storage.
     *
     * @param DeleteOpeningBalanceRequestNamespace $request
     * @param App\Models\openingBalance\OpeningBalance $opeingBalance
     * @return \App\Http\Responses\RedirectResponse
     */
    public function show($id, ManageOpeningBalanceRequest $request)
    {
        $opening = OpeningBalance::find($id);
        //returning with successfull message
        return new ViewResponse('focus.openingBalance.view', compact('opening'));
    }
    ...
    // {
    //     $subAccounts = Account::where('account_type', $id)->get();
