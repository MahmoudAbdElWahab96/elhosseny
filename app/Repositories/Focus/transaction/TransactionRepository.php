...
//        $input['credit'] = numberClean($input['debit']);
...
//        $account->balance = $account->balance + $input['debit'];
//        $account->balance = $account->balance - $input['debit'];
//        $account->save();
//
...
class TransactionRepository extends BaseRepository
{
    ...
    public function create(array $input)
    {
        if(@$input['cost_centers']){
            $costCenters = CostCenter::wherein('id', $input['cost_centers'])->get();
            foreach($costCenters as $costCenter){
                if($input['credit']){
                    $costCenter->update([
                        'cost_balance' => $costCenter->cost_balance - (float)$input['credit']
                    ]);
                }
                if($input['debit']){
                    $costCenter->update([
                        'cost_balance' => $costCenter->cost_balance + (float)$input['debit']
                    ]);
                }
            }
        }
        ...
        $account->balance = $account->balance - $input['debit'];
        $account->debit += $input['debit'];
        $account->credit += $input['credit'];
        $account->save();

        $account->CalcAccountBalance();
        ...
        if ($dual_entry['feature_value']) {
            ...
            $account2->balance = $account2->balance - $input2['debit'];
            $account2->debit += $input2['debit'];
            $account2->credit += $input2['credit'];
            $account2->save();

            $account2->CalcAccountBalance();
        }
