<div class="form-group">
    {{ Form::label( 'class', trans('additionals.tax_id'),['class' => 'col-lg-2 control-label']) }}
    <div class='col-lg-10'>
        <select class="form-control round" name="tax_id" id='tax_id'>
            @foreach($taxs as $tax)
                @if(app()->getLocale() == 'ar')
                    <option value="{{ $tax->id }}" {{ $tax->id === @$additionals->tax_id ? " selected" : "" }}>{{ $tax->code . '-'. $tax->Desc_ar }}</option>
                @else
                    <option value="{{ $tax->id }}" {{ $tax->id === @$additionals->tax_id ? " selected" : "" }}>{{ $tax->code . '-'. $tax->Desc_en}}</option>
                @endif
            @endforeach
        </select>
    </div>
...
<div class='form-group'>
    {{ Form::label( 'code', trans('additionals.code'),['class' => 'col-lg-2 control-label']) }}
    <div class='col-lg-10'>
        {{ Form::text('code', null, ['class' => 'form-control round', 'placeholder' => trans('additionals.code')]) }}
    </div>
</div>
<div class='form-group'>
...
<div class='form-group'>
    {{ Form::label( 'name_en', trans('additionals.name_en'),['class' => 'col-lg-2 control-label']) }}
    <div class='col-lg-10'>
        {{ Form::text('name_en', null, ['class' => 'form-control round', 'placeholder' => trans('additionals.name_en')]) }}
    </div>
</div>
