...
<div class="receipt">
    ...
    <p><small>
            ...
            <br>{{$general['lang_bill_date']}} : {{\Carbon\Carbon::parse($invoice->created_at)->format('Y-m-d , g:i a')}}
            <br>{{$general['person']}} : {{$invoice->customer->name}}
            <br>@lang('customers.phone') : {{$invoice->customer->phone}}
            <br>{{$general['person_address']}} : {{$invoice->customer->address}}
