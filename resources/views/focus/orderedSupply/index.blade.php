...
@section ('title', trans('labels.backend.orderedSupply.management'))
@section('page-header')
    <h1>{{ trans('labels.backend.orderedSupply.management') }}</h1>
@endsection
...
@section('content')
    ...
    <div class="">
        <div class="content-wrapper">
                ...
                <div class="content-header-right col-md-6 col-12">
                    <div class="media width-250 float-right">
                        ...
                        <div class="media-body media-right text-right">
                            ...
                            @include('focus.orderedSupply.partials.orderedSupply-header-buttons',$input)
                        </div>
                    </div>
                </div>
            </div>
            @if($segment)
                @php
                    $total=$segment->orderedSupply->sum('total');
                    $paid=$segment->orderedSupply->sum('pamnt');
                    $due=$total-$paid;
                @endphp
                <div class="card">
                        ...
                        <div class="card">
                            ...
                            <div class="card-content">
                                ...
                                <div class="card-body">
                                    ...
                                    <hr>
                                    <table id="orderedSupply-table_{{$input['meta']}}"
                                           class="table table-striped table-bordered zero-configuration" cellspacing="0"
                                           width="100%">
                                        <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>{{ trans('orderedSupply.orderedSupply')}} #{{prefix($input['pre'])}}</th>
                                            <th>{{ trans('customers.customer') }}</th>
                                            <th>{{ trans('general.amount') }}</th>
                                            <!-- <th>{{ trans('general.status') }}</th> -->
                                            <th>{{ trans('orderedSupply.orderedSupply_due_date') }}</th>
...
@section('after-scripts')
    ...
    <script>
        ...
        $(function () {
            ...
            $('#search').click(function () {
                ...
                if (start_date != '' && end_date != '') {
                    $('#orderedSupply-table_{{$input["meta"]}}').DataTable().destroy();
        ...
        function draw_data(start_date = '', end_date = '') {
            ...
            var dataTable = $('#orderedSupply-table_{{$input["meta"]}}').dataTable({
                processing: true,
                stateSave: true,
                serverSide: true,
                responsive: true,
                ...
                ajax: {
                    url: '{{ route("biller.orderedSupply.get") }}',
                    type: 'post',
                    data: {
                ...
                columns:
                    [
                        ...
                        {data: 'orderedSupplyduedate', name: 'orderedSupplyduedate'},
            ...
            $('#orderedSupply-table_{{$input['meta']}}_wrapper').removeClass('form-inline');
        }
    </script>
@endsection
