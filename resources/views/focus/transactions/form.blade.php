<div class='row'>
    <div class='col-md-6'>
        <div class='form-group'>
            ...
            <div class="col">
                <select name="account_id" class='form-control round' id="account_id">
                    @foreach($accounts as $account)
                        <option value="{{$account['id']}}">{{$account['holder'].' ('.$account['number'].') '}}</option>
                    @endforeach
                </select></div>
            <div id="cost_center_screens">
            ...
            <div class='form-group'>
                ...
                <div class="col">
                    <select name="account_id2" class='form-control round' id="account_id2">
                        @foreach($accounts as $account)
                            <option value="{{$account['id']}}">{{$account['holder'].' '.$account['number']}}</option>
                        @endforeach
                    </select></div>
...
@section("after-scripts")
    ...
    <script type="text/javascript">
        ...
        $("#account_id").on("change", function (e) {
            ...
            $.ajax({
                type: "GET",
                url: '{{ route("biller.transactions.getScreens") }}',
                data: {
                    account_id: account_id
                },
                success: function (data) {
                    $('#cost_center_screens').html(data);
